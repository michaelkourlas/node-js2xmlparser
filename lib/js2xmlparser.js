...
var init = function(root, data, options) {
    ...
    if (typeof options === "object" && options !== null) {
        ...
        if ("callMemberFunctions" in options) {
            if (typeof options.callMemberFunctions === "boolean")
                callMemberFunctions = options.callMemberFunctions;
            else
                throw new Error("callMemberFunctions option must be a boolean");
        }

        if ("excludedTypes" in options) {
            if (Array.isArray(options.excludedTypes) && typeof options.excludedTypes[0] === "string") {
                for (var i = 0; i < options.excludedTypes.length; ++i)
                    excludedTypes[options.excludedTypes[i]] = true;
            }
            else
                throw new Error("excludedTypes option must be an array of string");
        }
    }
...
var toString = function(data) {
    ...
    var functionHelper = function(data) {
        ...
        if (callMemberFunctions && Object.prototype.toString.call(data) === "[object Function]")
