...
(function() {
    ...
    var wrapElement = function(xml, element, level) {
        var tempObject = {};
        tempObject[wrapArrayItem] = element;
        return toXML(tempObject, xml, level);
    };
            ...
            if (Object.prototype.toString.call(object[property]) === "[object Array]") {
                if (wrapArray) {
                    ...
                    for (i = 0; i < object[property].length; i++) {
                        xml = wrapElement(xml, object[property][i], level + 1);
                    }
                    xml += addBreak(addIndent("</" + property + ">", level));
                }
                else {
                    ...
                    xml += addBreak(addIndent("<" + property + ">", level));
                    for (i = 0; i < object[property].length; i++) {
                        if (Object.prototype.toString.call(object[property][i]) === "[object String]") {
                            // Here we have no choice but to wrap the items
                            // And we can't simply put the index because it would not be a valid tag
                            xml = wrapElement(xml, object[property][i], level + 1);
                        } else {
                            xml = toXML(object[property][i], xml, level + 1);
                        }
                    }
                    xml += addBreak(addIndent("</" + property + ">", level));
                }
